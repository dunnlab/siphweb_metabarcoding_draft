...
for barcode in ${barcodes[@]}; do
  ...
  echo "module load BLAST/2.2.22-Linux_x86_64; module load USEARCH/6.1.544; module load HMMER/3.1b2-foss-2016a; metaxa2 -i $exportpath/$barcode-sequences-str.fasta -o $pipeline_dir/Q2_$barcode/METAXA2_$runid$barcode" -g SSU_SILVA123.1 -T 0,60,60,60,60,65,70,75,85,90,97 | cat >> "$pipeline_dir"/Q2_"$barcode"/METAXA2"$runid""$barcode"_jobfile.txt
  echo "module load BLAST/2.2.22-Linux_x86_64; module load USEARCH/6.1.544; module load HMMER/3.1b2-foss-2016a; metaxa2 -i $exportpath/$barcode-sequences-str.fasta -o $pipeline_dir/Q2_$barcode/METAXA2_default_$runid$barcode" -T 0,60,60,60,60,65,70,75,85,90,97 | cat >> "$pipeline_dir"/Q2_"$barcode"/METAXA2_default_"$runid""$barcode"_jobfile.txt
