...
Features <- read.csv(file = myargs[4], header = FALSE, sep = "\t")
outdir = myargs[5]
...
IDtable <- cbind(c("OTU",Ranks_pasted), c("Feature_name",Features_pruned[,1]), Features_pruned[,2:ncol(Features_pruned)])
...
IDtable<-IDtable[-1,]
print(sapply(IDtable[,1:2],class))
