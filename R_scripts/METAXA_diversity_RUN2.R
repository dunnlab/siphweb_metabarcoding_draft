...
setwd("/Volumes/GoogleDrive/My Drive/Metabarcoding_things/RUN2_results/")
...
rel_list = list()
for(i in 1:length(barcodes)){
  istring = paste("Q2_",barcodes[i],"/METAXA2_",runid, barcodes[i], ".taxonomy-reliability.txt", sep="")
  istring_def = paste("Q2_",barcodes[i],"/METAXA2_default_",runid, barcodes[i], ".taxonomy-reliability.txt", sep="")
...
#Eyeballing numbers
combinetab[which(combinetab$variable %>% str_count("Reso")>0), c(1,5)]
