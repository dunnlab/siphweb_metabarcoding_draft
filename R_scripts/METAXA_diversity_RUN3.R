...
for(i in 1:length(barcodes)){
  ...
  istring_def = paste(runid, barcodes[i], "IDtable_default.tsv", sep="")
  ...
  prunedlist[[i]] <- prunedlist[[i]][which(prunedlist[[i]]$value>0 & str_count(prunedlist[[i]]$OTU,"Metazoa")>0 & str_count(prunedlist[[i]]$OTU,"Hydroidolina")==0 & str_count(prunedlist[[i]]$OTU,"Mammalia")==0 & str_count(prunedlist[[i]]$OTU,"Metazoa__")==0),]
  prunedlist_def[[i]] <- prunedlist_def[[i]][which(prunedlist_def[[i]]$value>0 & str_count(prunedlist_def[[i]]$OTU,"Metazoa")>0 & str_count(prunedlist_def[[i]]$OTU,"Hydrozoa")==0 & str_count(prunedlist_def[[i]]$OTU,"Mammalia")==0 & str_count(prunedlist_def[[i]]$OTU,"Metazoa__")==0),]
}
...
rel_list = list()
for(i in 1:length(barcodes)){
  istring = paste("METAXA2_",runid, barcodes[i], ".taxonomy-reliability.txt", sep="")
  istring_def = paste("METAXA2_default_",runid, barcodes[i], ".taxonomy-reliability.txt", sep="")
...
for(i in 1:length(barcodes)){
  ...
  istring_def = paste("METAXA2_default_",runid, barcodes[i], ".taxonomy-table.tsv", sep="")
  tab_i <- read.csv(file = istring, header = FALSE, sep = "\t", stringsAsFactors = F)
  tab_i_def <- read.csv(file = istring_def, header = FALSE, sep = "\t", stringsAsFactors = F)
  tab_i_comb <- bind_rows(tab_i, tab_i_def)
  names(tab_i_comb) <- c("Feature_name", "IDs")
  tab_list[[i]] <- tab_i_comb
}

#Combine taxonomy onto OTUs
comblist_tab = comblist_rel
for(i in 1:length(barcodes)){
...
#Eyeballing numbers
combinetab[which(combinetab$OTU %>% str_count("Bivalvi")>0), c(1,5)]
